...
ARAD에서는 UAAL 필수 설정만 재정의하여, 빠르게 빌드하고 테스트할 수 있도록 유지·관리하였습니다. <br><br><br>

### Android ↔ Unity 간 데이터 송수신 방식
Unity와 Android는 각자 별도의 앱 프로세스로 동작하므로 `room` 또는 `SharedPreference` 공유가 어렵습니다.  
따라서 Unity와 Android 간 데이터 교환을 위해 제공되는 **UnityPlayer.UnitySendMessage** 등의 **양방향 인터페이스를 활용하여 통신** 구조를 구성하여 Android에서 Unity로 메시지를 보내거나, Unity에서 Android 메서드를 직접 호출하는 방식으로 구현하였습니다. <br><br><br>
